...
bool aligned_tensor(const at::Tensor& tensor){
  for(const auto i : c10::irange(tensor.dim() - 1)){
    if(tensor.stride(i) % alignment != 0){
      return false;
    }
  }
...
at::Tensor preprocess_mask(
  ...
  if (!aligned_tensor<mem_eff_alignment>(mask)) {
    TORCH_WARN(
        ...
        mem_eff_alignment,
        " bytes. "
        "Pad the attn_mask prior to calling SDPA by padding the last dimension "
        "to be a multiple of ", mem_eff_alignment,
        " and then slicing the attn_mask to the original size.");
